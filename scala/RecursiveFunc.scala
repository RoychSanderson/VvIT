...
object RecursiveFunctions {
  ...
  // Дерево работает в двух направлениях, так что явно нет


  def main(args: Array[String]): Unit = {
    println("reverse = " + reverse(Cons(4,Cons(2,Cons(5,Nil())))))
    println("map = " + map(Cons(4,Cons(2,Cons(5,Nil()))))((x: Int) => x * 2))
    println("append = " + append(Cons(1,Cons(2,Cons(3,Nil()))), Cons(4, Cons(5, Nil()))))
    println("flatMap = " + flatMap(Cons(1,Cons(2,Cons(3,Cons(4,Cons(5,Nil()))))))((x: Int) => Cons(x, Cons(x * 2, Nil()))))
  }
}
